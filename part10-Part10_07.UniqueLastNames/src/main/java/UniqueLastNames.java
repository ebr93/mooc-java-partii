...
public class UniqueLastNames {
    ...
    public static void main(String[] args) {
        ...
        // map gets only the last names in list to perform wanted action, distinct then finds the unique values, sorted puts them in alphabetical, for each prints them
        System.out.println("Unique last names in alphabetical order: ");
        persons.stream().map(pers -> pers.getLastName()).distinct().sorted().forEach(pers -> System.out.println(pers));
    }
}
